...
DMAMEM uint16_t screenBuffer[128 * 240];
...
void update_Master_Volume(void);
void send_MidiCC_USB(byte control_No, byte control_data);
void updateOsc1Parameter_a(uint16_t value);
void updateOsc1Parameter_b(uint16_t value);
void updateOsc2Parameter_a(uint16_t value);
void updateOsc2Parameter_b(uint16_t value);
void draw_wt_waterfall_view(uint8_t wt, uint8_t position, boolean cls);
...
void drawVoiceLED_mainpage(void)
{
	...
	tft.fillRect(xpos, ypos, 37, 17, ST7735_BLACK);

	for (uint8_t i = 0; i < 8; i++)
	{
		if (i < 4)
		{
			tft.drawRect(xpos + (i * 10), ypos, 8, 8, ST7735_BLUE);
		}
		else
		{
			tft.drawRect(xpos + ((i - 4) * 10), ypos_2, 8, 8, ST7735_BLUE);
		}
	}
	...
	for (uint8_t i = 0; i < 8; i++)
	{
		if (voices[i].voiceOn == 1 || VoicesLEDtime[i] != 0)
		{
			if (i < 4)
			{
				tft.fillRect(xpos + (i * 10), ypos, 8, 8, ST7735_BLUE);
			}
			else
			{
				tft.fillRect(xpos + ((i - 4) * 10), ypos_2, 8, 8, ST7735_BLUE);
			}
			VoicesLEDtime[i]--; // LED on time afther Voice off
		}
	}
...
void renderPeak()
{
	uint16_t time_10 = 55; // update time 55ms
	uint8_t BARnew = 0;
	static uint8_t BARold = 0;
	int BARval = 0;
	static boolean PeakFlag = false;
	static uint8_t PeakTime = 0;
	static uint16_t PeakPeakTime = 0;

	if ((millis() - timer10) > time_10)
	{
		timer10 = millis();
		// Peak Attack phase ------------------------------------------------
		if (vuMeter && peak.available())
		{
			...
			if (PeakPeakTime > 0)
			{
				...
				else
				{
					if (BARold >= 1)
					{
						tft.drawFastHLine(149, 12 - BARold, 9, ST7735_YELLOW);
					}
					else
					{
						tft.drawFastHLine(149, 12, 9, ST7735_GREEN);
					}
				}
				PeakPeakTime--;
			}
...
//*************************************************************************
// render Peakmeter on main page
//*************************************************************************
void renderPeak_2()
{
	uint16_t time_10 = 55; // update time 55ms
	uint8_t BARnew = 0;
	static uint8_t BARnew_ = 0;
	static uint8_t BARold = 0;
	int16_t BARval = 0;
	static boolean PeakFlag = false;
	static uint8_t PeakTime = 0;
	...
	if (MainShift == true && PageNr == 0) // if live parameter on screen active
	{
		return;
	}
	...
	if ((millis() - timer10) > time_10)
	{
		...
		if (vuMeter && peak.available())
		{
			BARval = (int16_t)(peak.read() * 1000.0f);
			...
			// clear old Bar line
			tft.fillRect(12, 71, 7, 49, ST7735_DARKGREY);

			// draw new Bar line
			tft.fillRect(12, 120 - BARnew, 7, BARnew, ST7735_GREEN);
			...
			{
				tft.drawFastHLine(12, ypos + (i * 5), 7, ST7735_BLACK);
			}

			// draw Peak
			if (BARnew >= 49)
			{
				tft.drawFastHLine(12, 71, 7, ST7735_RED);
				tft.drawFastHLine(12, 72, 7, ST7735_RED);
				tft.drawFastHLine(12, 73, 7, ST7735_RED);
				tft.drawFastHLine(12, 74, 7, ST7735_RED);
			}
			if (BARnew >= 45)
			{
				tft.drawFastHLine(12, 76, 7, ST7735_ORANGE);
				tft.drawFastHLine(12, 77, 7, ST7735_ORANGE);
				tft.drawFastHLine(12, 78, 7, ST7735_ORANGE);
				tft.drawFastHLine(12, 79, 7, ST7735_ORANGE);
			}
			if (BARnew >= 40)
			{
				tft.drawFastHLine(12, 81, 7, ST7735_YELLOW);
				tft.drawFastHLine(12, 82, 7, ST7735_YELLOW);
				tft.drawFastHLine(12, 83, 7, ST7735_YELLOW);
				tft.drawFastHLine(12, 84, 7, ST7735_YELLOW);
			}
			...
			if (PeakPeakTime > 0)
			{
				...
				{
					tft.drawFastHLine(12, 86, 7, ST7735_YELLOW);
				}
				else if (BARold >= 30)
				{
					tft.drawFastHLine(12, 91, 7, ST7735_YELLOW);
				}
				else if (BARold >= 25)
				{
					tft.drawFastHLine(12, 96, 7, ST7735_YELLOW);
				}
				else if (BARold >= 20)
				{
					tft.drawFastHLine(12, 101, 7, ST7735_YELLOW);
				}
				else if (BARold >= 15)
				{
					tft.drawFastHLine(12, 106, 7, ST7735_YELLOW);
				}
				else if (BARold >= 10)
				{
					tft.drawFastHLine(12, 111, 7, ST7735_YELLOW);
				}
				else if (BARold >= 5)
				{
					tft.drawFastHLine(12, 116, 7, ST7735_YELLOW);
				}
		...
		// Peak Release phase -----------------------------------------------
		if ((PeakFlag == true && PeakTime == 0) || PeakPeakTime == 1)
		{
			// clear Bar line
			tft.fillRect(12, 71, 7, 49, ST7735_DARKGREY);
			// draw LED line
		...
		tft.drawFastHLine(12, 119, 7, ST7735_GREEN);
	}
	// new VU bar
	else if (VUmeter_redraw == true)
	{
		tft.fillRect(12, 71, 7, 49, ST7735_DARKGREY);
		// draw LED line
		uint8_t count = 9;
...
{
	uint16_t frame_color = ST7735_YELLOW;
	// No frame refresh if parameter already selected
	if (PageNr_ == PageNr && ParameterNr_ == ParameterNr)
	{
		return;
	}

	PageNr_ = PageNr;
	ParameterNr_ = ParameterNr;
	...
	if (PageNr <= 2)
	{
		...
		// draw current Frame
		tft.drawRect(Xpos, 15 + (19 * col), width + 2, 18, frame_color);
	}
	else if (PageNr == 10)
	{
		uint8_t Xpos = 1;
	...
	else if (PageNr == 12 && myPageShiftStatus[PageNr] == true)
	{
		...
		// draw current Frame
		uint8_t col = ParameterNr;
		tft.drawRect(Xpos, 15 + (19 * col), width, hight, frame_color);
	}
}

//*************************************************************************
// print Braids wavetable name
// ************************************************************************
FLASHMEM void print_WTBL_name(uint16_t value)
{
	uint16_t string_id = 314;
	int16_t x1, y1;
	uint16_t w1, h1;

	tft.fillRect(85, 38, 72, 9, ST7735_BLACK);

	// get string_id from pot value
	if (value >= 1 && value <= 306)
	{
		string_id += ((value - 1) / 51);
	}
	else if (value >= 307 && value <= 562)
	{
		string_id += ((value - 2) / 51);
	}
	else if (value >= 563 && value <= 818)
	{
		string_id += ((value - 3) / 51);
	}
	else if (value >= 819)
	{
		string_id += ((value - 4) / 51);
	}

	char name[12];
	strcpy_P(name, (char *)pgm_read_dword(&(String_Tab[string_id])));
	tft.getTextBounds(name, 0, 0, &x1, &y1, &w1, &h1); // string width in pixels
	tft.setCursor(121 - (w1 / 2), 38);				   // print string in the middle
...
FLASHMEM void printModParameter(uint8_t ParameterNo)
{
	...
	else if (ParameterNr < 40)
	{
		print_String(139, 24, (39 + (0 * 19)));	 // print "MWHEEL"
		print_String(139, 24, (39 + (1 * 19)));	 // print "MWHEEL"
		print_String(139, 24, (39 + (2 * 19)));	 // print "MWHEEL"
		print_String(139, 24, (39 + (3 * 19)));	 // print "MWHEEL"
		print_String(284, 103, (39 + (0 * 19))); // print "Os1 PRM_A"
		print_String(285, 103, (39 + (1 * 19))); // print "Os1 PRM_B"
		print_String(286, 103, (39 + (2 * 19))); // print "Os2 PRM_A"
		print_String(287, 103, (39 + (3 * 19))); // print "Os2 PRM_B"
	}
	else if (ParameterNr < 44)
	{
		print_String(212, 24, (39 + (0 * 19)));	 // print "ATOUCH"
		print_String(212, 24, (39 + (1 * 19)));	 // print "MWHEEL"
		print_String(212, 24, (39 + (2 * 19)));	 // print "MWHEEL"
		print_String(212, 24, (39 + (3 * 19)));	 // print "MWHEEL"
		print_String(284, 103, (39 + (0 * 19))); // print "Os1 PRM_A"
		print_String(285, 103, (39 + (1 * 19))); // print "Os1 PRM_B"
		print_String(286, 103, (39 + (2 * 19))); // print "Os2 PRM_A"
		print_String(287, 103, (39 + (3 * 19))); // print "Os2 PRM_B"
	}
	else if (ParameterNr < 48)
	{
		print_String(452, 24, (39 + (0 * 19)));	 // print "ENV3"
		print_String(452, 24, (39 + (1 * 19)));	 // print "ENV3"
		print_String(452, 24, (39 + (2 * 19)));	 // print "ENV3"
		print_String(452, 24, (39 + (3 * 19)));	 // print "ENV3"
		print_String(284, 103, (39 + (0 * 19))); // print "Os1_PRMA"
		print_String(285, 103, (39 + (1 * 19))); // print "Os1_PRMB"
		print_String(286, 103, (39 + (2 * 19))); // print "Os2_PRMA"
		print_String(287, 103, (39 + (3 * 19))); // print "Os2_PRMB"
	}
	else if (ParameterNr < 52)
	{
		print_String(456, 24, (39 + (0 * 19)));	 // print "Drift"
		print_String(164, 24, (39 + (1 * 19)));	 // print ""
		print_String(164, 24, (39 + (2 * 19)));	 // print ""
		print_String(164, 24, (39 + (3 * 19)));	 // print ""
		print_String(457, 103, (39 + (0 * 19))); // print "Osc1+2"
		print_String(164, 103, (39 + (1 * 19))); // print ""
		print_String(164, 103, (39 + (2 * 19))); // print ""
...
FLASHMEM void printFxName(uint8_t PrgNo)
{
	...
	case 3:
		...
		break;
	case 4:
		tft.print("PITCH-SHIFT");
		break;
	case 5:
		tft.print("PITCH-ECHO");
		break;
	case 6:
		tft.print("REVERB 1");
		break;
	...
	case 11:
		...
		break;
	case 12:
		tft.print("FLANGERATOR");
		break;
	case 13:
		tft.print("DISTORTER");
		break;
	case 14:
		tft.print("STEREO-ECHO-REV");
		break;
	case 15:
		tft.print("GATE");
		break;
...
FLASHMEM void drawEnvCurve(uint8_t ATKvalue, uint8_t DCYvalue, uint8_t SUSvalue, uint8_t RELvalue)
{
	...
	if (PageNr == 4 && myPageShiftStatus[4] == 1)
	{
		k = 0.001f; // liniar curve in Wave envelope
	}
	else
	{
		k = -4.0f;
...
FLASHMEM void draw_Waveform(int WaveNr, uint16_t waveColor)
{
	...
	if (WaveNr == 0)
	{
		...
		tft.setFont(NULL);
		tft.fillRoundRect(8 + 30 + 8 + 30 + 8 + 30 + 8, 114, 30, 11, 2, 0x06E0);
		print_String(31, 129, 116); // print "---"
		return;
	}
	...
	if (WaveNr >= 13 && WaveBank == 0)
	{
		for (int i = 0; i < 64; i++)
		{
			...
			sample = 65535 - pgm_read_word(&(ArbBank[phase_x]));
			int16_t value = (int16_t)sample;
			// y2 = 35 - (value / 2941 );
			y2 = 35 - (value * 0.00034f);
			if (i == 0)
			{
				tft.drawLine(x1, y1, x2, y2, ST7735_BLACK);
			}
	...
	if (WaveBank >= 1 && WaveBank <= 14)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			sample = 65535 - pgm_read_word(&((ArbBank + WaveAddr)[phase_x]));
			int16_t value = (int16_t)sample;
			// y2 = 35 - (value / 2941 );
			y2 = 35 - (value * 0.00034f);
			if (i == 0)
	...
	if (WaveBank == 15)
	{
		...
		tft.setFont(NULL);
		// tft.fillRect(130,115,18,8,0x06E0);
		tft.fillRoundRect(8 + 30 + 8 + 30 + 8 + 30 + 8, 114, 30, 11, 2, 0x06E0);
		print_String(31, 129, 116); // print "---"
		...
		case 17:
			...
			break;
		case 18:
			print_String(297, 87, 24);	// BRAIDS
			print_String(309, 128, 24); // WTBL
			break;
		case 19:
			print_String(297, 101, 24); // BRAIDS
		...
		case 21:
			...
			break;
		case 22:
			print_String(341, 93, 24); // 8bit USER
			print_String(342, 87, 38); // WAVETABLE
			// tft.fillRect(135,115,18,8,ST7735_DARKGREEN);
			tft.fillRoundRect(8 + 30 + 8 + 30 + 8 + 30 + 8, 114, 30, 11, 2, 0x06E0);
			print_String(343, 132, 116); // print "WT"

			uint8_t user_wt_index_ = 0;
			if (PageNr == 1)
			{
				user_wt_index_ = user_wavetable_index[0];
			}
			else
			{
				user_wt_index_ = user_wavetable_index[1];
			}
...
//*************************************************************************
// draw complete Wavetables as 3D view
// wavetable position 0-63
//*************************************************************************
FLASHMEM void draw_wt_waterfall_view(uint8_t wt, uint8_t position, boolean cls)
{
	static uint8_t wave_count_ = 255;
	uint8_t ypos = 104;
	uint8_t sample = 0;
	uint8_t y1 = 0;
	uint8_t y2 = ypos;
	...
	uint8_t xx_ = 0;
	uint8_t y3 = ypos;
	uint32_t wt_addr = (129 * 64 * wt);
	float wt_gain = 0.10f;
	uint8_t wave_count = 0;
	uint32_t wt_addr_ = wt_addr;
	uint16_t wt_offset = 387;

	wt_screen_timer = millis();
	wt_screen_flag = true;

	// clear screen ?
	if (cls == true)
	{
		tft.fillRect(1, 15, 158, 94, ST7735_BLACK);
		tft.drawRect(1, 15, 158, 94, ST7735_WHITE);
		tft.fillRoundRect(121, 93, 33, 10, 2, ST7735_YELLOW);
		tft.setTextColor(ST7735_GRAY);
		print_String(449, 123, 95); // print "SHIFT"
		tft.setTextColor(ST7735_WHITE);
		print_String(450, 126, 83); // print "MENU"
	}

	if (wave_count_ != position / 3 || cls == true)
	{
		wave_count_ = position / 3;

		sample = pgm_read_byte(&(user_waves[wt_addr]));
		y1 = y3 - (sample * wt_gain);

		for (uint8_t i = 0; i < 21; i++)
		{
			for (uint8_t xpos = 0; xpos < 90; xpos++)
			{
				sample = pgm_read_byte(&(user_waves[(wt_addr + xx_)]));
				y2 = y3 - (sample * wt_gain);
				if (xpos == 0)
				{
					tft.drawLine(x1, y1, x2, y2, ST7735_BLACK);
				}
				...
				{
					tft.drawLine(x1, y1, x2, y2, ST7735_GRAY);
				}
			...
			// next wave
			wave_count++;
			wt_addr = wt_addr + wt_offset;
			sample = pgm_read_byte(&(user_waves[wt_addr]));
			y3 = ypos - (wave_count * 3);
			y1 = y3 - (sample * wt_gain);
			y2 = ypos - (wave_count * 3);
			x1 = 20 + (wave_count * 2);
			x2 = 20 + (wave_count * 2);
			x_fl = 0;
		}
		...
		// draw select wave line
		wt_offset = 387 * wave_count_;
		wt_addr_ = (129 * 64 * wt) + wt_offset;
		sample = pgm_read_byte(&(user_waves[wt_addr_]));
		ypos = 104;
		y3 = ypos - (wave_count_ * 3);
		y1 = y3 - (sample * wt_gain);
		x1 = 20 + (wave_count_ * 2);
		x2 = 20 + (wave_count_ * 2);
		y2 = ypos - (wave_count_ * 3);
		x_fl = 0;
		xx_ = 0;

		for (uint8_t xpos = 0; xpos < 90; xpos++)
		{
			sample = pgm_read_byte(&(user_waves[(wt_addr_ + xx_)]));
			y2 = y3 - (sample * wt_gain);
			if (xpos == 0)
			{
				tft.drawLine(x1, y1, x2, y2, ST7735_BLACK);
	...
	// print wave name and no
	tft.setTextColor(ST7735_ORANGE);
	uint16_t wt_string_no = 349 + wt;
	print_String(wt_string_no, 5, 19);
	tft.setCursor(5, 28);
	tft.print("Wave");
	tft.fillRect(32, 28, 11, 8, ST7735_BLACK);
	tft.setCursor(32, 28);
	tft.print(position);
}
		...
		if (count == 0 || count == 1 || count == 3 || count == 5)
		{
			for (int i = 0; i < 43; i++)
			{
				...
				if (i == 0)
				{
					...
					tft.drawLine(x1 + col, y1, x2 + col, y2, waveColor);
				x1++;
				x2 = x1;
				y1 = y2;
			}
...
FLASHMEM void drawLFOframe(uint8_t frameNo)
{
	...
	{
	case 0:
		tft.drawRect(2, 22, 50, 35, ST7735_YELLOW);
		break;
	case 1:
		tft.drawRect(54, 22, 50, 35, ST7735_YELLOW);
		break;
	case 2:
		tft.drawRect(106, 22, 50, 35, ST7735_YELLOW);
		break;
	case 3:
		tft.drawRect(2, 60, 50, 35, ST7735_YELLOW);
		break;
	case 4:
		tft.drawRect(54, 60, 50, 35, ST7735_YELLOW);
		break;
	case 5:
		tft.drawRect(106, 60, 50, 35, ST7735_YELLOW);
		break;
	}
}
...
FLASHMEM void update_Env3_Osc1_PRMA_mod(void)
{
	float amount = Env3Osc1PrmAAmt * DIV127;
	for (size_t i = 0; i < 8; i++)
	{
		Osc_Prm_mixer_A[i].gain(3, amount);
	}
}

FLASHMEM void update_Env3_Osc1_PRMB_mod(void)
{
	float amount = Env3Osc1PrmBAmt * DIV127;
	for (size_t i = 0; i < 8; i++)
	{
		Osc_Prm_mixer_B[i].gain(3, amount);
	}
}

FLASHMEM void update_Env3_Osc2_PRMA_mod(void)
{
	float amount = Env3Osc2PrmAAmt * DIV127;
	for (size_t i = 0; i < 8; i++)
	{
		Osc_Prm_mixer_C[i].gain(3, amount);
	}
}

FLASHMEM void update_Env3_Osc2_PRMB_mod(void)
{
	float amount = Env3Osc2PrmBAmt * DIV127;
	for (size_t i = 0; i < 8; i++)
	{
		Osc_Prm_mixer_D[i].gain(3, amount);
	}
}
...
FLASHMEM void renderModParameter(uint8_t ParameterNr, uint8_t value)
{
	...
	{ // LFO1 > Pitch
		...
		updateOscLFOAmt();
		send_MidiCC_USB(48, value);
	}
	else if (ParameterNr == 1)
	{ // LFO2 > Cutoff
		filterLfoAmt = LINEARLFO[value];
		updateFilterLfoAmt();
		send_MidiCC_USB(51, value);
	}
	else if (ParameterNr == 2)
	{ // LFO1*MW > Pitch
		MODWheelvalue = value;
	...
	else if (ParameterNr == 6)
	{ // OSC1 > Cutoff
		filterFM = (DIV127 * value);
		updateFilterFM();
	}
	else if (ParameterNr == 7)
	{ // OSC2 > Cutoff
	...
	else if (ParameterNr == 9)
	{ // LFO3 > FxPot1amt
		FxPot1amt = value;
	}
	else if (ParameterNr == 10)
	{ // LFO3 > FxPot2amt
		FxPot2amt = value;
	}
	else if (ParameterNr == 11)
	{ // LFO3 > FxPot3amt
		FxPot3amt = value;
	}
	else if (ParameterNr == 12)
	...
	else if (ParameterNr == 13)
	{ // LFO3 > FxMIXamt
		FxMIXamt = value;
	}
	else if (ParameterNr == 14)
	{ // Atouch > Cutoff
		AtouchAmt = value;
		updateAtouchCutoff();
	}
	else if (ParameterNr == 15)
	{ // Atouch > Pitch
	...
	else if (ParameterNr == 18)
	{ // Atouch > FxP1 Amt
		AtouchFxP1Amt = value;
	}
	else if (ParameterNr == 19)
	{ // Atouch > FxP2 Amt
		AtouchFxP2Amt = value;
	}
	else if (ParameterNr == 20)
	...
	else if (ParameterNr == 29)
	{ // LFO3 > Osc1 PRM_B Amt
		Lfo3Osc1PrmBAmt = value;
		update_LFO3_Osc1_PRMB_mod();
	}
	else if (ParameterNr == 30)
	{ // LFO3 > Osc2 PRM_A Amt
		Lfo3Osc2PrmAAmt = value;
		update_LFO3_Osc2_PRMA_mod();
	}
	else if (ParameterNr == 31)
	{ // LFO3 > Osc2 PRM_B Amt
		Lfo3Osc2PrmBAmt = value;
		update_LFO3_Osc2_PRMB_mod();
	}
	else if (ParameterNr == 32)
	{ // Filter Envelope > Osc1 PRM_A Amt
		filterEnvOsc1PrmAAmt = value;
		update_filterEnv_Osc1_PRMA_mod();
	}
	else if (ParameterNr == 33)
	{ // Filter Envelope > Osc1 PRM_B Amt
		filterEnvOsc1PrmBAmt = value;
		update_filterEnv_Osc1_PRMB_mod();
	}
	else if (ParameterNr == 34)
	{ // Filter Envelope > Osc2 PRM_A Amt
		filterEnvOsc2PrmAAmt = value;
		update_filterEnv_Osc2_PRMA_mod();
	}
	else if (ParameterNr == 35)
	{ // Filter Envelope > Osc2 PRM_B Amt
		filterEnvOsc2PrmBAmt = value;
		update_filterEnv_Osc2_PRMB_mod();
	}
	else if (ParameterNr == 36)
	{ // MWHEEL > Osc1 PRM_A
		ModWheelPRM_Aamt = value;
	}
	else if (ParameterNr == 37)
	{ // MWHEEL > Osc1 PRM_B
		ModWheelPRM_Bamt = value;
	}
	else if (ParameterNr == 38)
	{ // MWHEEL > Osc2 PRM_A
		ModWheelPRM_Aamt2 = value;
	}
	else if (ParameterNr == 39)
	{ // MWHEEL > Osc2 PRM_B
		ModWheelPRM_Bamt2 = value;
	}
	else if (ParameterNr == 40)
	{ // Aftertouch > Osc1 PRM_A
		AtouchPRM_Aamt = value;
	}
	else if (ParameterNr == 41)
	{ // Aftertouch > Osc1 PRM_B
		AtouchPRM_Bamt = value;
	}
	else if (ParameterNr == 42)
	{ // Aftertouch > Osc2 PRM_A
		AtouchPRM_Aamt2 = value;
	}
	else if (ParameterNr == 43)
	{ // Aftertouch > Osc2 PRM_B
		AtouchPRM_Bamt2 = value;
	}
	else if (ParameterNr == 44)
	{ // Env3 > Osc1 PRM_A
		Env3Osc1PrmAAmt = value;
		update_Env3_Osc1_PRMA_mod();
	}
	else if (ParameterNr == 45)
	{ // Env3 > Osc1 PRM_B
		Env3Osc1PrmBAmt = value;
		update_Env3_Osc1_PRMB_mod();
	}
	else if (ParameterNr == 46)
	{ // Env3 > Osc2 PRM_A
		Env3Osc2PrmAAmt = value;
		update_Env3_Osc2_PRMA_mod();
	}
	else if (ParameterNr == 47)
	{ // Env3 > Osc2 PRM_B
		Env3Osc2PrmBAmt = value;
		update_Env3_Osc2_PRMB_mod();
	}
	else if (ParameterNr == 48)
	{ // DRIFT > Osc1+2
		Osc_drift_value = value;
		updatesAllVoices();
	}
}
...
FLASHMEM uint8_t readModParameter(uint8_t ParameterNr)
{
	...
	{ // filterEnvelope > Osc1 PRM_A
		value = filterEnvOsc1PrmAAmt;
		return value;
	}
	else if (ParameterNr == 33)
	{ // filterEnvelope > Osc1 PRM_B
		value = filterEnvOsc1PrmBAmt;
		return value;
	}
	else if (ParameterNr == 34)
	{ // filterEnvelope > Osc2 PRM_A
		value = filterEnvOsc2PrmAAmt;
		return value;
	}
	else if (ParameterNr == 35)
	{ // filterEnvelope > Osc2 PRM_B
		value = filterEnvOsc2PrmBAmt;
		return value;
	}
	else if (ParameterNr == 36)
	{ // MWHEEL > Osc1 PRM_A
		value = ModWheelPRM_Aamt;
		return value;
	}
	else if (ParameterNr == 37)
	{ // MWHEEL > Osc1 PRM_B
		value = ModWheelPRM_Bamt;
		return value;
	}
	else if (ParameterNr == 38)
	{ // MWHEEL > Osc2 PRM_A
		value = ModWheelPRM_Aamt2;
		return value;
	}
	else if (ParameterNr == 39)
	{ // MWHEEL > Osc2 PRM_A
		value = ModWheelPRM_Bamt2;
		return value;
	}
	else if (ParameterNr == 40)
	{ // ATOUCH > Osc1 PRM_A
		value = AtouchPRM_Aamt;
		return value;
	}
	else if (ParameterNr == 41)
	{ // ATOUCH > Osc1 PRM_B
		value = AtouchPRM_Bamt;
		return value;
	}
	else if (ParameterNr == 42)
	{ // ATOUCH > Osc2 PRM_A
		value = AtouchPRM_Aamt2;
		return value;
	}
	else if (ParameterNr == 43)
	{ // ATOUCH > Osc2 PRM_B
		value = AtouchPRM_Bamt2;
		return value;
	}
	else if (ParameterNr == 44)
	{ // ENV3 > Osc1 PRM_A
		value = Env3Osc1PrmAAmt;
		return value;
	}
	else if (ParameterNr == 45)
	{ // ENV3 > Osc1 PRM_B
		value = Env3Osc1PrmBAmt;
		return value;
	}
	else if (ParameterNr == 46)
	{ // ENV3 > Osc2 PRM_A
		value = Env3Osc2PrmAAmt;
		return value;
	}
	else if (ParameterNr == 47)
	{ // ENV3 > Osc3 PRM_B
		value = Env3Osc2PrmBAmt;
		return value;
	}
	else if (ParameterNr == 48)
	{ // DRIFT > Osc1+2
		value = Osc_drift_value;
		return value;
	}
	else
		return 0;
...
FLASHMEM void renderCurrentParameter(uint8_t Page, uint16_t ParameterNr, uint16_t value)
{
	...
	else if (Page == 11)
	{
		...
		if (ParameterNr == mySEQdiv)
		{
			...
			{
				static uint8_t val_ = 0;
				uint8_t val = (value * 0.11f);
				if (val >= 12)
				{
					val = 12;
				}
				if (val != val_)
				{
					...
					printSEQclkDiv(val);
					printRedMarker(3, value);
					uint32_t gtime = (SEQclkRate / SEQGateTime);
					if (gtime <= 3200)
					{
						gtime = 3200;
					}
			...
			else
			{
				...
				value = value >> 1;
				uint8_t val = (value * 0.1f);
				if (val == 0)
				{
					MidiClkDiv = 48;
					SEQdivValue = 2.0f; // 1/2
				}
				else if (val == 1)
				{
					MidiClkDiv = 48;
	...
	else if (Page == 12)
	{
		...
		if (myPageShiftStatus[PageNr] == false)
		{
			...
			// TX-CHA (Midi Transmit Channel)
			else if (ParameterNr == myMidiTransmitCha)
			{
				value = (value * 0.133f);
				if (value != midi_transmit_Channel)
				{
					midi_transmit_Channel = value;
					storeMidiTransmitChannel(midi_transmit_Channel);
					tft.fillRoundRect(133, 38, 22, 10, 2, ST7735_BLUE);
					tft.setTextColor(ST7735_WHITE);
					if (midi_transmit_Channel == 0)
					{
						tft.setCursor(136, 40);
						tft.print("OFF");
					}
					else
					{
						tft.setCursor(137, 40);
						tft.print(midi_transmit_Channel);
					}
				}
			}

			// RX-CC (Midi Control data)
			else if (ParameterNr == myUSBctrl)
			{
				uint8_t val;
				if (value >= 64)
				{
				...
				if (val != USBctrl)
				{
					USBctrl = val;
					storeUSBctrl(val);
					tft.fillRoundRect(132, 57, 24, 10, 2, ST7735_BLUE);
					tft.setTextColor(ST7735_WHITE);
					if (val == 0)
					{
					...
					{
						print_String(33, 136, 59); // print "ON"
					}
				}
			}

			// PCHANGE (Receive Midi Program Change)
			else if (ParameterNr == myPRGChange)
			{
				tft.fillRoundRect(132, 76, 24, 10, 2, ST7735_BLUE);
				tft.setTextColor(ST7735_WHITE);
				value = value >> 6;
				if (value == 0)
				{
					print_String(18, 136, 78); // print "OFF"
				}
				else
				{
					print_String(33, 136, 78); // print "ON"
				}
			...
			else if (ParameterNr == myVelocity)
			{
				...
				tft.fillRoundRect(132, 95, 24, 10, 2, ST7735_BLUE);
				tft.setTextColor(ST7735_WHITE);
...
FLASHMEM void print_quadsaw_pwamt(void)
{
	...
	else if (PageNr == 1 && (Osc1WaveBank == 15 && oscWaveformA >= 1))
	{
		if (oscWaveformA == 18)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(335, 85, 78); // print "WTBL"
		}
		else if (oscWaveformA == 19)
		{
			print_String(336, 85, 59); // print "X POS"
			print_String(337, 85, 78); // print "Y POS"
		}
		else if (oscWaveformA == 20 || oscWaveformA == 22)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(338, 85, 78); // print "SMOOTH"
		}
		else if (oscWaveformA == 21)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(339, 85, 78); // print "CHORD"
		}
		else
		{
		...
		if (oscWaveformA == 1 || oscWaveformA >= 8)
		{
			tft.fillRoundRect(133, 57, 22, 10, 2, ST7735_BLUE);
			tft.fillRoundRect(133, 76, 22, 10, 2, ST7735_BLUE);
			tft.setTextColor(ST7735_WHITE);
	...
	else if (PageNr == 2 && (Osc2WaveBank == 15 && oscWaveformB >= 1))
	{
		...
		else if (oscWaveformB == 19)
		{
			print_String(336, 85, 59); // print "X POS"
			print_String(337, 85, 78); // print "Y POS"
		}
		else if (oscWaveformB == 20 || oscWaveformB == 22)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(338, 85, 78); // print "SMOOTH"
		}
		else if (oscWaveformB == 21)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(339, 85, 78); // print "CHORD"
		}
		...
		if (oscWaveformB == 1 || oscWaveformB >= 8)
		{
			tft.fillRoundRect(133, 57, 22, 10, 2, ST7735_BLUE);
			tft.fillRoundRect(133, 76, 22, 10, 2, ST7735_BLUE);
			tft.setTextColor(ST7735_WHITE);
...
void drawMainPage(void)
{
	...
	if (RefreshMainScreenFlag == true)
	{
		PickUpflag = true;
		// clear PatchNo
		tft.fillRect(23, 9, 47, 18, ST7735_BLACK);
		// clear Patch Name
		tft.fillRect(0, 40, 160, 20, ST7735_BLACK);
		// print bank nr
		const uint8_t ypos = 13;
		tft.fillRoundRect(6, 9, 16, 20, 2, ST7735_RED);
		tft.setTextColor(ST7735_WHITE);
		tft.setFont(&FreeSans9pt7b);
		tft.setTextSize(1);
		tft.setCursor(xposBankNo[currentPatchBank] + 2, ypos);
		...
		tft.drawFastHLine(10, 63, 139, ST7735_RED);
		// tft.getTextBounds(currentPatchName, 0, 0, &x1, &y1, &w1, &h1); // string width in pixels
		tft.setCursor(9, 42); // print string in the middle
		tft.setTextColor(ST7735_WHITE);
		tft.println(currentPatchName);
		...
		if (SeqSymbol == true)
		{
			tft.fillRoundRect(126, 19, 23, 10, 2, ST7735_GREEN);
			tft.setTextColor(ST7735_BLACK);
		}
		else
		{
			tft.fillRoundRect(126, 19, 23, 10, 2, ST7735_GRAY);
			tft.setTextColor(ST77XX_DARKGREY);
		}
		tft.setFont(0);
		tft.setCursor(129, 20);
		tft.print("SEQ");
		printVoiceMode();
	}
	...
	if (myPageShiftStatus[0] == false)
	{
		...
		tft.fillRect(25, 65, 126, 68, ST7735_BLACK); // clear Scope screen
		tft.setFont(NULL);
		tft.setTextColor(ST7735_WHITE);
	...
	if (clearVu == true)
	{
		tft.fillRect(0, 65, 126, 68, ST7735_BLACK); // clear Scope screen
		clearVu = false;
	}
	...
	if (mute_sound == true)
	{
		tft.fillRect(25, 65, 160, 38, ST7735_BLACK); // clear Scope screen
		tft.setTextColor(ST7735_WHITE);
		/*
		for (uint8_t i = 0; i < 8; i++)
		{
	...
	{
		// tft.drawFastHLine(26, 93, 121, ST7735_GRAY);
		enableScope(true);
		// draw Voices LED
...
FLASHMEM void drawOsc1Page(void)
{
	...
	{ // Osc1 Main Menu
		...
		else if (Osc1WaveBank == 15 && oscWaveformA >= 1 && oscWaveformA <= 21)
		{
			print_String(289, 85, 59); // print "PRM_A"
			print_String(290, 85, 78); // print "PRM_B"
		}
		else if (Osc1WaveBank == 15 && oscWaveformA == 22)
		{
			print_String(334, 85, 59); // print "POS"
			print_String(338, 85, 78); // print "SMOOTH"
		}
		print_String(8, 85, 97); // print "OSCMIX"
		tft.fillRoundRect(8, 114, 30, 12, 2, ST7735_ORANGE);
		tft.fillRoundRect(8 + 30 + 8, 114, 30, 12, 2, ST7735_RED);
		tft.fillRoundRect(8 + 30 + 8 + 30 + 8, 113, 30, 12, 2, ST7735_BLUE);
		tft.fillRoundRect(8 + 30 + 8 + 30 + 8 + 30 + 8, 114, 30, 11, 2, 0x06E0);
		tft.setTextColor(ST7735_WHITE);
		print_String(29, 14, 116); // print "SEL"
		print_String(30, 52, 116); // print "SET"
		print_String(36, 88, 116); // print "BANK"
		if (Osc1WaveBank == 15 && oscWaveformA == 22)
		{
			print_String(343, 138, 115); // print "WT"
		}
		else
			print_String(31, 129, 116); // print "---"
...
FLASHMEM void drawOscSubMenu()
{
	...
	tft.setTextColor(ST7735_WHITE);
	tft.fillRoundRect(8, 114, 30, 12, 2, ST7735_ORANGE);
	tft.fillRoundRect(46, 114, 30, 12, 2, ST7735_RED);
	tft.fillRoundRect(84, 113, 30, 12, 2, ST7735_BLUE);
	tft.fillRoundRect(122, 114, 30, 11, 2, 0x06E0);
	tft.setTextColor(ST7735_WHITE);
	print_String(29, 14, 116);	// print "SEL"
	print_String(30, 52, 116);	// print "SET"
	print_String(31, 90, 116);	// print "---"
	print_String(31, 129, 116); // print "---"
...
FLASHMEM void draw_StateVariable_Filter_page(void)
{
	...
	{ // SUB page enabled
		...
		print_String(37, 5, 3); // print "FILTER"
		tft.setTextColor(ST7735_GREEN);
		print_String(38, 5, 19); // print "STATE"
		tft.setTextColor(ST7735_WHITE);
	...
	{ // SUB page enabled
		...
		tft.setTextColor(ST7735_WHITE);
		print_String(40, 9, 103);	 // print "CUT"
		print_String(41, 50, 103);	 // print "RES"
...
FLASHMEM void draw_Ladder_Filter_page(void)
{
	...
	{ // Main Page
		...
		print_String(37, 5, 3); // print "FILTER"
		tft.setTextColor(ST7735_YELLOW);
		print_String(39, 5, 19); // print "LADDER"
		tft.setTextColor(ST7735_WHITE);
		print_String(40, 10, 103);	// print "CUT"
		print_String(41, 50, 103);	// print "RES"
		print_String(42, 90, 103);	// print "ENV"
...
//*************************************************************************
// draw Filter_Env and Wavtable_Env Page
//*************************************************************************
FLASHMEM void drawFilterEnvPage(void)
{
	...
	// draw vcf_env parameters ------------------------------
	if (myPageShiftStatus[PageNr] == false)
	{
		print_String(52, 5, 3);	 // print "VCF"
		print_String(42, 25, 3); // print "ENV"
		// tft.fillRoundRect(70, 21, 80, 11, 2, ST7735_ORANGE);
		tft.setTextColor(ST7735_ORANGE);
		print_String(453, 5, 22); // print "Filter-Envelope"
		tft.setTextColor(ST7735_LIGHTGRAY);
		print_String(35, 45, 3); // print "SUB"
	...
	// draw Wavetable Envelop parameters -----------------------------
	else
	{
		tft.setTextColor(ST7735_WHITE);
		print_String(451, 5, 3); // print "WAV"
		print_String(42, 25, 3); // print "ENV"
		// tft.fillRoundRect(126, 25, 21, 11, 2, ST7735_BLUE);
		// print_String(451, 128, 28); // print "WAV"
		tft.setTextColor(ST77XX_YELLOW);
		print_String(454, 5, 22); // print "WAVE-Envelope"
		tft.setTextColor(ST7735_LIGHTGRAY);
		print_String(35, 45, 3); // print "SUB"
		...
		tft.setTextSize(0);
		tft.setTextColor(ST7735_WHITE);
		printDataValue(0, Env3Atk);
		printRedMarker(0, Env3Atk);
		printDataValue(1, Env3Dcy);
		printRedMarker(1, Env3Dcy);
		printDataValue(2, Env3Sus);
		printRedMarker(2, Env3Sus);
		printDataValue(3, Env3Rel);
		printRedMarker(3, Env3Rel);
...
FLASHMEM void drawAmpEnvPage(void)
{
	...
	print_String(455, 5, 22); // print "AMP-Envelope"
	tft.setTextColor(ST7735_LIGHTGRAY);
	print_String(35, 45, 3); // print "SUB"
	...
	else
	{
		...
		print_String(313, 130, 103); // print "AMP"
		uint8_t Value;
		// print Curve value (+-8)
		...
		uint8_t Mark_val_ = (MasterVolume * 25.4f);
		if (Mark_val_ >= 127)
		{
			Mark_val_ = 127;
		}
		printRedMarker(3, Mark_val_);
	}

	// draw Envelope curve, Midi symbol and peakmeter
	drawEnvCurve(Env2Atk, Env2Dcy, Env2Sus, Env2Rel);
	draw_Voice_State();
	MidiStatusSymbol = 2;
	MidiSymbol();
...
FLASHMEM void drawModMatrixPage(void)
{
	...
	tft.fillRoundRect(122, 114, 30, 11, 2, 0x06E0);
	tft.setTextColor(ST7735_WHITE);
	print_String(174, 11, 116); // print "SLOT"
	print_String(177, 52, 116); // print "AMT"
	print_String(31, 90, 116);	// print "---"
	print_String(31, 129, 116); // print "---"
...
FLASHMEM void drawSystemPar1(void)
{
	...
	tft.setTextColor(ST7735_WHITE);
	print_String(140, 85, 21); // print "RX-CHA"
	print_String(344, 85, 40); // print "TX-CHA"
	print_String(340, 85, 59); // print "RX-CC"
	print_String(144, 85, 78); // print "PCHANGE"
	print_String(141, 85, 97); // print "VELCURV"

	for (uint8_t i = 0; i < 5; i++)
	{
		tft.fillRoundRect(132, 19 + (19 * i), 24, 10, 2, ST7735_BLUE);
	}

	tft.setTextColor(ST7735_WHITE);

	// RX_CHA (Midi Receive Channel) -------------------
	if (midiChannel == 0)
	{
	...
	// TX_CHA (Midi Transmit Channel) -------------------
	if (midi_transmit_Channel == 0)
	{
		print_String(18, 136, 40); // print "OFF"
	}
	else
	{
		tft.setCursor(137, 40);
	...
	// RX-CC (receive Midi control data) -------------------
	if (USBctrl == 0)
	{
		print_String(18, 136, 59); // print "OFF"
	}
...
FLASHMEM void printVoiceMode(void)
{
	...
	if (PageNr == 0)
	{
		tft.setFont(0);
		tft.setTextColor(ST7735_WHITE);
		tft.fillRoundRect(125, 8, 25, 10, 2, ST7735_BLACK); // ST7735_BLUE    0x001

		if (unison == 0)
		{
			print_String(235, 126, 10); // print "POLY"
		}
		else if (unison == 1)
		{
			switch (Voice_mode)
			{
			case 0:
				print_String(240, 126, 10);
				break; // print "MONO"
			case 1:
				print_String(241, 126, 10);
				break; // print "MON2"
			case 2:
				print_String(242, 126, 10);
				break; // print "MON3"
			case 3:
				print_String(243, 126, 10);
				break; // print "MON4"
			case 4:
				print_String(244, 126, 10);
				break; // print "MON6"
			case 5:
				print_String(245, 126, 10);
				break; // print "CHOR"
			}
		}
	...
	else if (PageNr == 12)
	{
		...
		if (unison == 1)
		{
			...
			{
			case 0:
				print_String(240, 132, 97);
				break; // print "MONO"
			case 1:
				print_String(241, 132, 97);
				break; // print "MON2"
			case 2:
				print_String(242, 132, 97);
				break; // print "MON3"
			case 3:
				print_String(243, 132, 97);
				break; // print "MON4"
			case 4:
				print_String(244, 132, 97);
				break; // print "MON6"
			case 5:
				print_String(245, 132, 97);
				break; // print "CHOR"
			}
		}
	}
...
{
	PageNr_ = 255; // refresh yellow parameter frame
...
void draw_scope_line(void)
{
	...
	{
		tft.drawFastHLine(26, 93, 121, ST7735_GRAY);
		prev_pixel_y = map(scope_buffer[0], 32767, -32768, -63, 63) + 93;
		if (prev_pixel_y < 64)
			prev_pixel_y = 64;
		if (prev_pixel_y >= 122)
			prev_pixel_y = 122;
		...
		{
			pixel_y = map(scope_buffer[i], 32767, -32768, -63, 63) + 93;
			if (pixel_y <= 64)
				pixel_y = 64;
			if (pixel_y >= 122)
				pixel_y = 122;
			tft.drawLine(pixel_x + 26, prev_pixel_y, pixel_x + 27, pixel_y, 0x07B0);
			prev_pixel_y = pixel_y;
			pixel_x += 2;
			i++;
		}
		tft.drawFastHLine(26, 64, 121, ST7735_BLACK);
		tft.drawFastHLine(26, 122, 121, ST7735_BLACK);
	}
	else
	{
		tft.drawFastHLine(19, 83, 121, ST7735_GRAY);
		prev_pixel_y = map(scope_buffer[0], 32767, -32768, -42, 42) + 83;
		if (prev_pixel_y <= 65)
			prev_pixel_y = 65;
		if (prev_pixel_y >= 101)
			prev_pixel_y = 101;
		...
		for (uint8_t i = 0; i < 120; i++)
		{
			pixel_y = map(scope_buffer[i], 32767, -32768, -39, 39) + 83;
			if (pixel_y <= 65)
				pixel_y = 65;
			if (pixel_y >= 101)
				pixel_y = 101;
			tft.drawLine(pixel_x + 19, prev_pixel_y, pixel_x + 20, pixel_y, 0x07B0);
			prev_pixel_y = pixel_y;
			pixel_x += 2;
			i++;
		}
...
FLASHMEM void setupDisplay()
{
	...
	for (uint8_t i = 0; i < 1; i++)
	{
		...
		delay(waitTime);
		bmpDraw("PIC/PIC9.bmp", 0, 0);
		get_play_voice(note + 12, velocity);
		delay(waitTime);
		bmpDraw("PIC/PIC8.bmp", 0, 0);
		delay(waitTime);
		bmpDraw("PIC/PIC7.bmp", 0, 0);
