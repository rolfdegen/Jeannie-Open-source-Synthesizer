...
//*************************************************************************
// draw complete Wavetables as waterfall view
...
FLASHMEM void renderModParameter(uint8_t ParameterNr, uint8_t value)
{
	...
	else if (ParameterNr == 23) {	// KEY > LOUDNESS
	...
	else if (ParameterNr == 36) {	// MWHEEL > Osc1 PRM_A
		ModWheelPRM_Aamt = value;
	}
	else if (ParameterNr == 37) {	// MWHEEL > Osc1 PRM_B
		ModWheelPRM_Bamt = value;
	}
	else if (ParameterNr == 38) {	// MWHEEL > Osc2 PRM_A
		ModWheelPRM_Aamt2 = value;
	}
	else if (ParameterNr == 39) {	// MWHEEL > Osc2 PRM_B
		ModWheelPRM_Bamt2 = value;
	}
	else if (ParameterNr == 40) {	// Aftertouch > Osc1 PRM_A
		AtouchPRM_Aamt = value;
	}
	else if (ParameterNr == 41) {	// Aftertouch > Osc1 PRM_B
		AtouchPRM_Bamt = value;
	}
	else if (ParameterNr == 42) {	// Aftertouch > Osc2 PRM_A
		AtouchPRM_Aamt2 = value;
	}
	else if (ParameterNr == 43) {	// Aftertouch > Osc2 PRM_B
		AtouchPRM_Bamt2 = value;
	}
	else if (ParameterNr == 44) {	// Env3 > Osc1 PRM_A
		Env3Osc1PrmAAmt = value;
		update_Env3_Osc1_PRMA_mod();
	}
	else if (ParameterNr == 45) {	// Env3 > Osc1 PRM_B
		Env3Osc1PrmBAmt = value;
		update_Env3_Osc1_PRMB_mod();
	}
	else if (ParameterNr == 46) {	// Env3 > Osc2 PRM_A
		Env3Osc2PrmAAmt = value;
		update_Env3_Osc2_PRMA_mod();
	}
	else if (ParameterNr == 47) {	// Env3 > Osc2 PRM_B
		Env3Osc2PrmBAmt = value;
		update_Env3_Osc2_PRMB_mod();
	}
	else if (ParameterNr == 48) {	// DRIFT > Osc1+2
...
FLASHMEM uint8_t readModParameter(uint8_t ParameterNr)
{
	...
	else if (ParameterNr == 39) {	// MWHEEL > Osc2 PRM_A
		value = ModWheelPRM_Bamt2;
		return value;
	}
	else if (ParameterNr == 40) {	// ATOUCH > Osc1 PRM_A
		value = AtouchPRM_Aamt;
		return value;
	}
	else if (ParameterNr == 41) {	// ATOUCH > Osc1 PRM_B
		value = AtouchPRM_Bamt;
		return value;
	}
	else if (ParameterNr == 42) {	// ATOUCH > Osc2 PRM_A
		value = AtouchPRM_Aamt2;
		return value;
	}
	else if (ParameterNr == 43) {	// ATOUCH > Osc2 PRM_B
	...
	else if (ParameterNr == 47) {	// ENV3 > Osc3 PRM_B
...
FLASHMEM void drawOsc1Page (void)
{
	...
	if (myPageShiftStatus[PageNr] == false) {	// Osc1 Main Menu
		...
		else print_String(31,129,116);	// print "---"
...
FLASHMEM void printVoiceMode(void) {
	...
	if (PageNr == 0) {
		...
		if (unison == 0) {
			...
			switch(Voice_mode) {
				case 0: print_String(240,126,10); break;		// print "MONO"
				case 1: print_String(241,126,10); break;		// print "MON2"
				case 2: print_String(242,126,10); break;		// print "MON3"
				case 3: print_String(243,126,10); break;		// print "MON4"
				case 4: print_String(244,126,10); break;		// print "MON6"
				case 5: print_String(245,126,10); break;		// print "CHOR"
			}
		}
...
void displayThread()
{
	const uint16_t Time_1 = 50; // render pages
	const uint16_t Time_2 = 65; // Screen refresh	
	...
	// wavetable screen on time
	if ((millis() - wt_screen_timer) > wt_on_time)
	{
