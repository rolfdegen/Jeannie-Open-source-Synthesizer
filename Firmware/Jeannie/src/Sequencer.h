...
void send_Midi_NoteOn(byte note, byte velocity, byte channel);
void send_Midi_NoteOff(byte note, byte velocity, byte channel);
			...
			if (ARPSEQstatus == 0 && SEQrunStatus == true) {
				...
				if (SeqNoteBufStatus[SEQselectStepNo] == 1) {
					...
					for (uint8_t i = 0; i < noteCount; i++) {
						...
						send_Midi_NoteOn(myNote, velo, midi_transmit_Channel);
					}
				}
			}
		}
		...
		if ((micros() - timer_intMidiClk) > gateTime) {
			if (ARPSEQstatus == true && SEQrunStatus == true) {
				...
				if (SeqNoteBufStatus[SEQselectStepNo] == 1) {
					uint8_t noteCount = SeqNoteCount[SEQselectStepNo];
					for (uint8_t i = 0; i < noteCount; i++) {
						int midi_note = SeqNote1Buf[(SEQselectStepNo + (i * 16))];
						myNoteOff(midiChannel, midi_note, 0);
						send_Midi_NoteOff(midi_note, 0, midi_transmit_Channel);
					}
				}
