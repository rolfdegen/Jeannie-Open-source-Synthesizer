...
void handleSendMidiNoteOff(byte note, byte velocity, byte channel);
...
void myNoteOff(byte channel, byte note, byte velocity) {
	...
	// if Midi sustain mode ----------------------------------------------------------
	if (Midi_hold_flag == true && SEQrunStatus == false)
	{
		voice_nr = getVoiceNo(note);		// get voice_nr
		voiceSustain[voice_nr - 1] = true;	// set sustain_flag for voice_nr
		return;
	}
	
	// Poly Mode ----------------------------------------------------------
	if (unison == 0) {
		if (Midi_hold_flag == false)
		{
			voice_nr = getVoiceNo(note);				// get voice_nr
			if (voice_nr != -1)
			{
				voiceSustain[voice_nr - 1] = false;			// clear sustain flag for voice_nr
				handlePolyphonicNoteOff(note, velocity); 	// set note off
			}
		}
		else
		{
			voice_nr = getVoiceNo(note);
			if (voice_nr != -1)
			{
				if (voiceSustain[voice_nr - 1] == true)
				{
					handlePolyphonicNoteOff(note, velocity);
					voiceSustain[voice_nr - 1] = false;
				}
			}
		}
		return;
...
void handlePolyphonicNoteOn (uint8_t note, uint8_t velocity) {
	...
	// if sustain dissabled or sequencer run than play voice
	if (Midi_hold_flag == false || SEQrunStatus == true)
	{
		get_play_voice(note, velocity);
	}
	
	else
	{	// if sustain enabled than check note into voice buffer
		voice_nr = getVoiceNo(note);		// if same note into voice buffer dont play
		if (voice_nr == -1)
		{
			get_play_voice(note, velocity);
		}
...
void play_voices(int voice, byte note, byte velocity, float level) {
	...
	float loudness = (POWER[note] * ((POWER[KEY_mod_LOUDNESS]) * 1.8));
	if (loudness >= 1.9f)
	{
		loudness = 1.9f;
	}
...
FLASHMEM void play_monoVoices(int voice, byte note, byte velocity, float level, float pan_a, float pan_b) {
	...
	LFO2Envelope[voice].noteOn();
	LFO3EnvelopeAmp.noteOn();
	ampEnvelope[voice].releaseNoteOn(1.0f);
	ampEnvelope[voice].delay(6.0f);	
}
...
void updateVoice(uint8_t voice_no, uint8_t notesOn, uint8_t note)
{
	...
	// random drift value. Osc_drift_value is a parameter (Slot 49) in modulation menu
	float osc_drift_ = random(-75, 75);
	osc_drift_ = 1 + (osc_drift_ * 0.00000093f * Osc_drift_value); 
	
	// Poly mode
	if (unison == 0)
	{
		// update oscillator frequency
		waveformModa[voice_no].frequency(NOTEFREQS[voices[voice_no].note + pitchNotea] * oscMasterTune * osc_drift_);
		waveformModb[voice_no].frequency(NOTEFREQS[voices[voice_no].note + pitchNoteb] * detune * oscMasterTune * osc_drift_);
	}
	...
	else if (unison == 1 && Voice_mode <= 4)
	{
		...
		{
		case 1:
			waveformModa[0].frequency(NOTEFREQS[note + pitchNotea] * oscMasterTune * osc_drift_);
			waveformModb[0].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.04f))) * oscMasterTune * osc_drift_);
			break;
		case 2:
			waveformModa[0].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.09f))) * oscMasterTune * osc_drift_);
			waveformModb[0].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.14f))) * oscMasterTune * osc_drift_);
			waveformModa[1].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.09f))))) * oscMasterTune * osc_drift_);
			waveformModb[1].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.14f))))) * oscMasterTune * osc_drift_);
			break;
		case 3:
			waveformModa[0].frequency(NOTEFREQS[note + pitchNotea] * oscMasterTune * osc_drift_);
			waveformModb[0].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.04f))) * oscMasterTune * osc_drift_);
			waveformModa[1].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.09f))))) * oscMasterTune * osc_drift_);
			waveformModb[1].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.14f))))) * oscMasterTune * osc_drift_);
			waveformModa[2].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.09f))) * oscMasterTune * osc_drift_);
			waveformModb[2].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.14f))) * oscMasterTune * osc_drift_);
			break;
		case 4:
			waveformModa[0].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.09f))) * oscMasterTune * osc_drift_);
			waveformModb[0].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.14f))) * oscMasterTune * osc_drift_);
			waveformModa[1].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.70f))) * oscMasterTune * osc_drift_);
			waveformModb[1].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.74f))) * oscMasterTune * osc_drift_);
			waveformModa[2].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.09f))))) * oscMasterTune * osc_drift_);
			waveformModb[2].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.14f))))) * oscMasterTune * osc_drift_);
			waveformModa[3].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.70f))))) * oscMasterTune * osc_drift_);
			waveformModb[3].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.74f))))) * oscMasterTune * osc_drift_);
			break;
		case 6:
			waveformModa[0].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.019f))) * oscMasterTune * osc_drift_);
			waveformModb[0].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.024f))) * oscMasterTune * osc_drift_);
			waveformModa[1].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.41f))) * oscMasterTune * osc_drift_);
			waveformModb[1].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.45f))) * oscMasterTune * osc_drift_);
			waveformModa[2].frequency((NOTEFREQS[note + pitchNotea] * (detune + ((1 - detune) * 0.77f))) * oscMasterTune * osc_drift_);
			waveformModb[2].frequency((NOTEFREQS[note + pitchNoteb] * (detune + ((1 - detune) * 0.81f))) * oscMasterTune * osc_drift_);
			waveformModa[3].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.76f))))) * oscMasterTune * osc_drift_);
			waveformModb[3].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.80f))))) * oscMasterTune * osc_drift_);
			waveformModa[4].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.40f))))) * oscMasterTune * osc_drift_);
			waveformModb[4].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.44f))))) * oscMasterTune * osc_drift_);
			waveformModa[5].frequency((NOTEFREQS[note + pitchNotea] * (1 + (1 - (detune + ((1 - detune) * 0.02f))))) * oscMasterTune * osc_drift_);
			waveformModb[5].frequency((NOTEFREQS[note + pitchNoteb] * (1 + (1 - (detune + ((1 - detune) * 0.025f))))) * oscMasterTune * osc_drift_);
			break;
		}
	}
	// Chord mode
	else if (unison == 1 && Voice_mode == 5)
	{
		...
		{
			waveformModa[i].frequency((NOTEFREQS[note + pitchNotea + CHORD_DETUNE[i][chordDetune]]) * oscMasterTune * osc_drift_);
			waveformModb[i].frequency((NOTEFREQS[note + pitchNoteb + CHORD_DETUNE[i][chordDetune]] * CDT_DETUNE) * oscMasterTune * osc_drift_);
		}
	}
}
	...
	if (unison == 1)
	{
		...
		*/
		...
		case 1: // Mono-2
			...
			amp = ((0.5 - myAmpVelocity) + (velo * myAmpVelocity)) * gain_;
			amp = amp * 2;
		...
		case 5: // Chord
			...
			pan_a = 0.5f + pan; // left pan side
			pan_b = 0.5f - pan; // right pan side
			voiceMixer1b.gain(2, amp * pan_a);
			voiceMixer2b.gain(2, amp * pan_b);
			voiceMixer1b.gain(3, amp * pan_b);
			voiceMixer2b.gain(3, amp * pan_a);
			break;
		}
	}
}