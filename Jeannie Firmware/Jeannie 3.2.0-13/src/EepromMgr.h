...
#define EEPROM_MIDI_TRANS_CH 58
...
FLASHMEM int getMIDITransChannel() {
  byte midiTransChannel = EEPROM.read(EEPROM_MIDI_TRANS_CH);
  if (midiTransChannel < 0 || midiTransChannel > 16) midiTransChannel = MIDI_CHANNEL_OMNI;//If EEPROM has no MIDI channel stored
  midi_transmit_Channel = midiTransChannel;
  return midi_transmit_Channel;
}
...
FLASHMEM void storeMidiTransmitChannel(byte channel){
  EEPROM.update(EEPROM_MIDI_TRANS_CH, channel);
}
