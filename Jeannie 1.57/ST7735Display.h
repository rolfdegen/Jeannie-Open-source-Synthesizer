...
FLASHMEM void renderCurrentParameter(uint8_t Page,uint16_t ParameterNr, uint8_t value)
{
	...
	// Page:11 System  ----------------------------------------------------
		...
		if (myPageShiftStatus[PageNr] == false) {
			...
			// Pickup
			if (ParameterNr == myPickup) {
				tft.fillRoundRect(133,19,22,10,2,ST7735_BLUE);
				tft.setTextColor(ST7735_WHITE);
				value = value >> 6;
				if (value == 0) {
					print_String(18,136,21);	// print "OFF"
					} else {
					print_String(33,138,21);	// print "ON"
				}
				if (pickupFlag != value) {
					pickupFlag = value;
...
FLASHMEM void printPickupValue (uint8_t pot, uint8_t value, uint8_t prevValue, boolean prevFlag)
{
	...
	if (prevFlag == false && value != prevValue) {
		...
		val = roundf(value * DIV100);   // 100/127 div
		...
		// print current value
		val = round(prevValue * DIV100);	// 100/127 div
		if (val < 10) {
			tft.setCursor(xpos + 6, ypos);
		}
		else if ( val < 100) {
...
{ 
	...
	if (myFilter == 1) {
		...
		if (myPageShiftStatus[PageNr] == false) {
			...
			PrevValue[0] = Frq_value >> 1;
			PrevValue[1] = FilterReso;
			PrevValue[2] = EnvVal;
			PrevValue[3] = FilterMix;
			if (pickupFlag == true) {
				printPickupValue (0, Frq_value >> 1, potVal1 >> 5, 0);
				printPickupValue (1, FilterReso, potVal2 >> 5, 0);
				printPickupInt64 (2, EnvVal, potVal3 >> 5, 0);
				printPickupVCFTyp(3, FilterMix, potVal4 >> 5, 0);
			} else {
				printPickupValue (0, Frq_value >> 1,Frq_value >> 1, 0);
				printPickupValue (1, FilterReso, FilterReso, 0);
				printPickupInt64 (2, EnvVal, EnvVal, 0);
				printPickupVCFTyp(3, FilterMix, FilterMix, 0);
			}
		}
		...
		// SUB Page enabled
			...
			// calc Filter Keytraking
			uint8_t myFilterKey = roundf(FilterKey * DIV100x);
			...
			int potVal2 = MCP_adc.read(MCP3208::Channel::SINGLE_1);
			int potVal3 = MCP_adc.read(MCP3208::Channel::SINGLE_2);
			PrevValue[0] = myFilterKey;
			PrevValue[1] = myVelo;
			PrevValue[2] = Amtvelo;
			if (pickupFlag == true) {
				printPickupValue (0, myFilterKey, potVal1 >> 5, 0);
				printPickupValue (1, myVelo, potVal2 >> 5, 0);
				printPickupValue (2, Amtvelo, potVal3 >> 5, 0);
				} else {
				printPickupValue (0, myFilterKey, myFilterKey, 0);
				printPickupValue (1, myVelo, myVelo, 0);
				printPickupValue (2, Amtvelo, Amtvelo, 0);
			}
			// Pot 4 non 
			printDataValue (3, 0);
			printRedMarker (3, 0);
		...
		if (myPageShiftStatus[PageNr] == false) {
			...
			PrevValue[1] = FilterReso;
			PrevValue[2] = EnvVal;
			PrevValue[3] = LadderFilterDrive;
			if (pickupFlag == true) {
				printPickupValue (0, Frq_value >> 1, potVal1 >> 5, 0);
				printPickupValue (1, FilterReso, potVal2 >> 5, 0);
				printPickupInt64 (2, EnvVal, potVal3 >> 5, 0);
				printPickupValue(3, LadderFilterDrive, potVal4 >> 5, 0);
			} else {
				printPickupValue (0, Frq_value >> 1, Frq_value >> 1, 0);
				printPickupValue (1, FilterReso, FilterReso, 0);
				printPickupInt64 (2, EnvVal,EnvVal, 0);
				printPickupValue(3, LadderFilterDrive, LadderFilterDrive, 0);
			}
		...
		// SUB Page enabled -------------------------------------
			...
			*/
			...
			PrevValue[1] = myVelo;
			PrevValue[2] = Amtvelo;
			PrevValue[3] = LadderFilterpassbandgain;
			if (pickupFlag == true) {
				printPickupValue (0, myFilterKey, potVal1 >> 5, 0);
				printPickupValue (1, myVelo, potVal2 >> 5, 0);
				printPickupValue (2, Amtvelo, potVal3 >> 5, 0);
				printPickupValue (3, LadderFilterpassbandgain, potVal4 >> 5, 0);
				} else {
				printPickupValue (0, myFilterKey, myFilterKey, 0);
				printPickupValue (1, myVelo, myVelo, 0);
				printPickupValue (2, Amtvelo, Amtvelo, 0);
				printPickupValue (3, LadderFilterpassbandgain, LadderFilterpassbandgain, 0);
			}
		}
	}
...
{ 
	...
	if (myPageShiftStatus[PageNr] == false) {
		...
		Env1Sus = SUSvalue;
		Env1Rel = RELvalue;
		PrevValue[0] = ATKvalue;
		PrevValue[1] = DCYvalue;
		PrevValue[2] = SUSvalue;
		PrevValue[3] = RELvalue;
		if (pickupFlag == true) {
			printEnvATKvaluesX(ATKvalue, mux1Read >> 5, PrevFlag[0]);
			printEnvDCYvaluesX(DCYvalue, mux2Read >> 5, PrevFlag[1]);
			printEnvSUSvaluesX(SUSvalue, mux3Read >> 5, PrevFlag[2]);
			printEnvRELvaluesX(RELvalue, mux4Read >> 5, PrevFlag[3]);
		} else {
			printEnvATKvaluesX(ATKvalue, ATKvalue, PrevFlag[0]);
			printEnvDCYvaluesX(DCYvalue, DCYvalue, PrevFlag[1]);
			printEnvSUSvaluesX(SUSvalue, SUSvalue, PrevFlag[2]);
			printEnvRELvaluesX(RELvalue, RELvalue, PrevFlag[3]);
		}
	}
	
	// SUB menu -----------------------------------------------------------
	else {
		uint8_t curVal = 128 - (64 - (envelopeType1 * 7.48f));
		
		Serial.print("envelopeType1: ");
		Serial.println(envelopeType1);
		Serial.print("curVal: ");
		...
		if (pickupFlag == true) {
			printPickupInt8 (0, curVal, potVal1 >> 5, 0);
			} else {
			printPickupInt8 (0, curVal, curVal, 0);
		}
	...
	// update Midi Symbol and peakmeter -----------------------------------
...
FLASHMEM void drawSystemPar2 (void)
{
	...
	tft.setTextColor(ST7735_GRAY);
	print_String(201,85,21);	// print "Pick up"
	print_String(31,98,40);		// print "---"
	print_String(31,98,59);		// print "---"
	print_String(31,98,78);		// print "---"
	print_String(31,98,97);		// print "---"
...
FLASHMEM void drawSubPageInfo () {
	...
	uint8_t xpos[12] = {0,32,32,45,45,45,0,32,32,26,0,45};	// Text Position
	
	if (PageNr != 0 && PageNr != 6 && PageNr != 10) {
		if (myPageShiftStatus[PageNr] == true) {
			if (blink == true) {
...
FLASHMEM void drawProgressbar() {
	...
	// draw fill bar
	if (ProgBar_style == 1) {
		tft.fillRect(25,61,value,15,ST7735_GRAY);	// send = Black
		} 
